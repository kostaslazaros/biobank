...
def series_update(request, series):
    ...
    if request.method == "POST":
        form = SeriesUpdateForm(
            request.POST, request.FILES, instance=matching_series)
        if form.is_valid():
            form.save()
            return redirect('homepage')
...
def article_update(request, series, article):
    ...
    if request.method == "POST":
        form = ArticleUpdateForm(
            request.POST, request.FILES, instance=matching_article)
        if form.is_valid():
            form.save()
...
def upload_image(request, series, article):
    ...
    file_path = os.path.join(
        settings.MEDIA_ROOT, 'ArticleSeries', matching_article.slug, file_obj.name)
